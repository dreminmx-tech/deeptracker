...
[![Bundle](https://img.shields.io/badge/бандл-85%20kB%20gzip-333538?style=flat-square&labelColor=141418)](docs/technical.md#build)
...
npm test         # 67 тестов: чистая логика + рендер всех экранов
```
...
- 📓 **Журнал, который читается как чат** — старые дни сверху, сегодняшний внизу, а поле для заметки прижато под ним, с одной кнопкой отправки. Enter делает перенос строки (он для этого и нужен), каждое нажатие клавиши уже сохранено, поэтому закрытое приложение ничего не теряет, а клавиатура поднимает поле над собой, а не закрывает его. Заметка правится тапом по ней и не удаляется совсем — именно поэтому случайное движение пальцем ничего не стоит. Любой прошлый день открывается кнопкой «+», если туда нужно что-то дописать.
